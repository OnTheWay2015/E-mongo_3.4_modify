...
void BSONObj::init(const char* data) {
    _objdata = data;
    if (!isValid())
        _assertInvalid();
    ...
    BSONObj::BSONObj(ConstSharedBuffer ownedBuffer)
        : _objdata(ownedBuffer.get() ? ownedBuffer.get() : BSONObj().objdata()),
          _ownedBuffer(std::move(ownedBuffer)) {
        desc = toString(); 
    }

    BSONObj& BSONObj::operator=(BSONObj otherCopy) noexcept {
        this->swap(otherCopy);
        return *this;
    }

    /** Swap this BSONObj with 'other' */
    void BSONObj::swap(BSONObj& other) noexcept {
        using std::swap;
