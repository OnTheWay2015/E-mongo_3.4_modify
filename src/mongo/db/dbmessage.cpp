    ...
    if (v >= 2000 && v < 2100) {
        ...
        switch (v) {
            case dbUpdate: {
                ...
                ss << " [dbUpdate] flags: " << flags << " query: " << q << " update: " << o;
                break;
            }
            case dbInsert:
                ss << " [dbInsert] " << d.nextJsObj();
                break;
            case dbDelete: {
                int flags = d.pullInt();
                BSONObj q = d.nextJsObj();
                ss << " [dbDelete] flags: " << flags << " query: " << q;
                break;
            }
            case dbMsg: {
                ss << " [dbMsg] todo";
            ...
            case dbQuery: {
                 int numberToSkip = d.pullInt();  // 设置从第一个document起，跳过（忽略）的document数
                int numberToReturn = d.pullInt();// 返回的document数 
                BSONObj q = d.nextJsObj();
                ss << " [dbQuery] numberToSkip: " << numberToSkip << ", numberToReturn:"<< numberToReturn <<",  query: " << q ;
                break;
            }
            default: {
